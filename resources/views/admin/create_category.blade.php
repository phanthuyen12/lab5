...
function select_data(){
    ...
    $('#list_data').append(`
        <tr>
            ...
            <td>${item.trangthai === 1 ? 'Ẩn' : 'Hiện'}</td> 
            <td>${item.created_at}</td>
            <td>${item.products_count}</td>
            <td>
                <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#modal_${item.id}" onclick="get_item_id(${item.id})">Cập Nhật</button>
                <div class="modal fade" id="modal_${item.id}" tabindex="-1" role="dialog" aria-labelledby="modal_${item.id}_title" aria-hidden="true">
                    <div class="modal-dialog modal-xl" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="modal_${item.id}_title">Cập nhật</h5>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
                            <div class="modal-body">
                                <form id="user_registration_form_${item.id}" action="/admin/update_category" method="post" enctype="multipart/form-data">
                                    @csrf
                                    <div class="form-group">
                                        <label for="name_category_${item.id}">Tên Danh Mục</label>
                                        <input type="text" class="form-control" id="name_category_${item.id}" name="name_category" aria-describedby="emailHelp" placeholder="${item.name_category}" value="${item.name_category}">
                                        <input type="hidden" name="id" id="id_${item.id}" value="${item.id}">
                                    </div>
                                    <div class="form-group">
                                        <label for="category_img_${item.id}">Hình</label>
                                        <input type="file" class="form-control" id="category_img_${item.id}" name="category_img" aria-describedby="emailHelp">
                                    </div>
                ...
                <button type="button" class="btn btn-danger btn-sm" onclick="update_trangthai(${item.id})">Cập Nhật Trạng Thái</button>
                <button type="button" class="btn btn-danger btn-sm" onclick="delete_item(${item.id})">Xóa</button>
            </td>
        </tr>
    `);
